...
/**
 * \file    wpshop/class/thirdparty_wpshop.class.php
 * \ingroup wpshop
...
/**
 * Class ThirdPartyWpshop
 */

class ThirdPartyWpshop {
	public function __construct() {}

	public function checkThirdPartyExistOnWp($object) {
		global $user;
		...
		if (! empty($object->array_options['options__wps_id']))
		{
			...
			if (!$response) {
				...
				{
					setEventMessages('WP Third Party was not found, delete wps_id entry.', null);
					return 0;
				}
			}
		}
	...
	public function createThirdPartyOnWp($object) {
		global $user;
		...
		if ($response['status']) {
			...
			{
				setEventMessages('WP Product was created, updated Dolibarr Third Party with wps_id #' . $response['data']['wp_object']['data']['id'], null);
				return 0;
			}
		} else {
