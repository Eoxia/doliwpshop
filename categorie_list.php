...
$search_wpshop_id = trim(GETPOST('search_wpshop_id', 'alpha'));
$search = array();
...
// Custom WPShop setup
$arrayfields['nb_products'] = array('label'=>'Nombre de produits', 'checked'=>1, 'enabled'=>1, 'position'=>90);
$arrayfields['wpshop_id'] = array('label'=>'WPshop ID', 'checked'=>1, 'enabled'=>1, 'position'=>91);
// Ensure native extrafield is hidden to avoid duplicate
if (isset($arrayfields['ef._wps_id'])) {
	$arrayfields['ef._wps_id']['checked'] = 0;
}
...
// --------------------------------------------------------------------
$sql = "SELECT";
$sql .= " (SELECT COUNT(fk_product) FROM ".$db->prefix()."categorie_product WHERE fk_categorie = t.rowid) as nb_products,";
...
if ($search_wpshop_id != '') {
	$sql .= natural_search('ef._wps_id', $search_wpshop_id, 1);
}
...
if ($mode == 'hierarchy') {
	...
	if ($search_wpshop_id != '') {
		$param .= '&search_wpshop_id='.urlencode($search_wpshop_id);
	}
	...
	// Custom WPShop search
	if (!empty($arrayfields['nb_products']['checked'])) {
		print '<td class="liste_titre"></td>'; // No search on this computed column for now
	}
	if (!empty($arrayfields['wpshop_id']['checked'])) {
		print '<td class="liste_titre">';
		print '<input type="text" class="flat maxwidth75" name="search_wpshop_id" value="'.dol_escape_htmltag(isset($search['wpshop_id']) ? $search['wpshop_id'] : '').'">';
		print '</td>';
	}
	...
	// Custom WPShop headers
	if (!empty($arrayfields['nb_products']['checked'])) {
		print getTitleFieldOfList($arrayfields['nb_products']['label'], 0, $_SERVER['PHP_SELF'], 'nb_products', '', $param, '', $sortfield, $sortorder, 'center ')."\n";
	}
	if (!empty($arrayfields['wpshop_id']['checked'])) {
		print getTitleFieldOfList($arrayfields['wpshop_id']['label'], 0, $_SERVER['PHP_SELF'], 'wpshop_id', '', $param, '', $sortfield, $sortorder, 'center ')."\n";
	}
	...
	while ($i < $imaxinloop) {
		...
		if ($mode == 'kanban') {
			...
			// Custom WPShop output
			if (!empty($arrayfields['nb_products']['checked'])) {
				print '<td class="center">'.$obj->nb_products.'</td>';
			}
			if (!empty($arrayfields['wpshop_id']['checked'])) {
				print '<td class="center">';
				if (!empty($obj->options__wps_id)) {
					$wp_url = !empty($conf->global->WPSHOP_URL_WORDPRESS) ? rtrim($conf->global->WPSHOP_URL_WORDPRESS, '/') : '';
					if ($wp_url) {
						$link = $wp_url . '/wp-admin/term.php?taxonomy=wps-product-cat&tag_ID=' . $obj->options__wps_id . '&post_type=wps-product';
						print '<a href="'.$link.'" target="_blank" rel="noopener noreferrer">'.$obj->options__wps_id.'</a>';
					} else {
						print $obj->options__wps_id;
					}
				}
				print '</td>';
			}
